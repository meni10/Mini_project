...
def get_gmail_service(user=None):
    ...
    try:
        ...
        if user and user.is_authenticated:
            ...
            # For anonymous users, get the most recent credentials
        ...
        # Test the service by fetching the user's profile
        try:
            profile = service.users().getProfile(userId='me').execute()
            logger.info(f"Successfully connected to Gmail API for user: {profile.get('emailAddress')}")
            
            # If we have an anonymous user with credentials, but the user is now authenticated,
            # update the credentials to link to the authenticated user
...
def _load_creds_from_db(user=None):
    ...
    try:
        ...
        if user and user.is_authenticated:
            ...
            # For anonymous users, get the most recent anonymous credential
            creds_obj = GmailCredentials.objects.filter(user=None).order_by('-updated_at').first()
        ...
        if isinstance(scopes, str):
            try:
                ...
                # If that fails, try splitting by space (as in OAuth URL)
                scopes = scopes.split()
    ...
    try:
        ...
        for message in messages:
            try:
                ...
                to_email = next((h['value'] for h in headers if h['name'] == 'To'), '')  # Add 'to' field
                ...
                emails.append({
                    'id': message['id'],
                    'threadId': msg.get('threadId', ''),  # Add threadId
                    'subject': subject,
                    'from': from_email,
                    'to': to_email,  # Add to field
                    'date': date,
                    'snippet': snippet,
                    'body_text': body_text  # Add body_text field
                })
            except Exception as e:
...
# In services/gmail.py - replace the existing fetch_drafts function with this:
